...
export default FeedScreen = (props) => {
    ...
    useEffect(() => {
      const getDb = async () => {
        try {
          const value = await AsyncStorage.getItem('firstInstalledFeed')
          // 값이 존재하면
          if(value !== null) {
            setModalPage(value)
          }
          // 값이 존재하지 않으면
            ...
            await AsyncStorage.setItem('firstInstalledFeed', '1');
            setModalPage('1');
          }
        } catch(e) {
          console.log(e)
    ...
    const insertDb = async (value) => {
      try {
        await AsyncStorage.setItem('firstInstalledFeed', value)
      } catch (e) {
        console.log(e)
      }
    }
    ...
    useEffect(() => {
      ...
      modalPage == '1' ? showInfo() : null 
    }, [modalPage])
    
    const [tooltipSize, setTooltipSize] = useState({width: 100, height: 50})
    
    const tooltipClone = React.cloneElement(
      <Text>심부름을 요청할 수 있어요</Text>,
