...
export default MyErrandScreen = (props) => {
    ...
    const [firstInstalled, setFirstInstalled] = useState(false);
    ...
    // 가변 tooltip 크기 조절
    const [tooltipSize, setTooltipSize] = useState({width: 150, height: 50})
    const tooltipClone = React.cloneElement(
      <Text>현재 내가 진행중인 심부름을 확인할 수 있어요</Text>,
    ...
    // tooltip 관련
    useEffect(() => {
      // 값이 존재하지 않으면 false 저장 및 tooltip 실행
      const getDb = async () => {
        const val = await AsyncStorage.getItem('firstInstalledMyErrand');
        if (val !== null) {
          setFirstInstalled(val)
        } else {
          await AsyncStorage.setItem('firstInstalledMyErrand', JSON.stringify(false))
          setFirstInstalled(true)
        }
      }
      // tooltip 실행 함수
      const tooltipOn = () => requestAnimationFrame(() => {
        console.log('tooltipOn 실행')
        tooltipRef.current.toggleTooltip();
      })

      getDb();
      if (firstInstalled == true) tooltipOn(); else null;
    ...
    const renderTabBar = (props) => (
        <Tooltip
          ...
          width={tooltipSize.width + 30} //220
          height={tooltipSize.height + 30} //100
          popover={tooltipClone}
          skipAndroidStatusBar={true}
          containerStyle={{borderRadius: 20, shadowRadius: 4, backgroundColor: Colors.lightGray}}
